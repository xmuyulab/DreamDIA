...
drf_dim = 16
score_ids = [i for i in range(3 * n_base_scores + n_simple_scores + drf_dim + 2) if i != 2]
...
def feature_construction(dream_score_res, lib_cols, top_k):  
    ...
    assay_rt_scores = np.array(list(dream_score_res[ASSAY_RT_COL].apply(get_top_scores)))
    ...
    assay_rt_scores_mean = assay_rt_scores.mean(axis = 1)
    ...
    mean_scores_list = [delta_rt_scores_mean, delta_rt_square_scores_mean, dream_scores_mean, lib_cos_scores_mean, assay_rt_scores_mean]
    all_mean_scores = np.vstack(mean_scores_list).T
    assay_rt_scores_std = assay_rt_scores.std(axis = 1)
    ...
    std_scores_list = [delta_rt_scores_std, delta_rt_square_scores_std, dream_scores_std, lib_cos_scores_std, assay_rt_scores_std] 
    all_std_scores = np.vstack(std_scores_list).T
    all_scores_list = [delta_rt_scores, delta_rt_square_scores, dream_scores, lib_cos_scores, assay_rt_scores, all_mean_scores, all_std_scores]
    ...
    all_scores_df["score%d" % n_scores] = dream_score_res[lib_cols["PURE_SEQUENCE_COL"]].apply(lambda x : len(x)) # 15
    all_scores_df["score%d" % (n_scores+1)] = dream_score_res[lib_cols["PRECURSOR_MZ_COL"]]  # 16
    all_scores_df["score%d" % (n_scores+2)] = dream_score_res[lib_cols["PRECURSOR_CHARGE_COL"]]  # 17
    all_scores_df["score%d" % (n_scores+3)] = dream_score_res[lib_cols["IRT_COL"]]  # 18
    original_columns = list(dream_score_res.columns)
    local_score_columns = list(all_scores_df.columns)
    all_data = pd.concat([dream_score_res, all_scores_df], axis = 1, ignore_index = True)
    all_data.columns = original_columns + local_score_columns    
    return all_data
def data_augmentation(all_data, top_k):
    ...
    for i in range(top_k):
        ...
        other_score_columns = ["score%d" % k for k in range(top_k * n_base_scores, top_k * n_base_scores + (2 * n_base_scores + n_simple_scores))]
...
def feature(dream_score_res, lib_cols, top_k, score0_cutoff, score2_cutoff):
    ...
    def get_score_string(x, y):
        return x.strip().split(";")[y - 1]
    def get_score(x, y):
        return float(x.strip().split(";")[y - 1])
    augmented_data["drf_string"] = augmented_data.apply(lambda df: get_score_string(df[DRF_SCORES_COL], df["peak_group_rank"]), axis = 1)
    augmented_data["ms2_area_string"] = augmented_data.apply(lambda df: get_score_string(df[MS2_AREA_COL], df["peak_group_rank"]), axis = 1)
    for i in range(drf_dim):
        augmented_data["score%d" % (n_base_scores * 3 + n_simple_scores + i)] = augmented_data["drf_string"].apply(lambda x : float(x.strip().split("|")[i]))
    augmented_data["score%d" % (n_base_scores * 3 + n_simple_scores + drf_dim)] = augmented_data.apply(lambda df: get_score_string(df[MS1_AREA_COL], df["peak_group_rank"]), axis = 1)
    augmented_data["score%d" % (n_base_scores * 3 + n_simple_scores + drf_dim + 1)] = augmented_data["ms2_area_string"].apply(lambda x : float(x.strip().split("|")[0]))
    return augmented_data 
...
def dream_indicators_rf(augmented_data, lib_cols, n_threads, seed, output_path, logger, fdr_precursor):
    ...
    logger.info("Optimal max depth: %d" % optimal_depth)
    return optimal_depth
def dream_prophet(dream_score_res, lib_cols, disc_model, top_k, n_threads, seed, dream_indicators, disc_dir, logger, fdr_precursor, fdr_protein, score0_cutoff, score2_cutoff):
    if not os.path.exists(disc_dir):
        os.mkdir(disc_dir)
    augmented_data = feature(dream_score_res, lib_cols, top_k, score0_cutoff = score0_cutoff, score2_cutoff = score2_cutoff)
    if dream_indicators:
        if disc_model == "xgboost":
    ...
    fdr_data.to_csv(os.path.join(disc_dir, "DreamDIA_fdr_results.tsv"), sep = "\t", index = False)
    ...
    protein_data.to_csv(os.path.join(disc_dir, "DreamDIA_protein_fdr_results.tsv"), sep = "\t", index = False)
